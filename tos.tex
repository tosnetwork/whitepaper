...
\title{
  ...
  \large Typed, Authorized, Metered, Receipted and Settleable Operations\\
...
\begin{abstract}
AI agents are moving from assistants that recommend actions to economic actors that invoke
services, spend budgets and coordinate across organizational boundaries. The interface layer
is improving quickly, but a service endpoint and a payment instruction do not by themselves
establish who an agent represents, what it may do, how much it may consume, what evidence it
must return or how failure is resolved.

TOS Network is being built as \textbf{The Open System for the Agentic Internet}: a portable
authorization, metering, receipt and settlement layer through which independently operated
agents can safely invoke one another and external services. TOS models meaningful Internet
actions---including messaging, model and tool calls, storage, events, booking, commerce and
physical-edge work---as versioned \textbf{Agentic Service Operations}. Each operation has an
explicit identity, input and output contract, capability scope, replay domain, resource
envelope, maximum budget, metering rules, receipt semantics and settlement policy.

An \textbf{Agentic Service Opcode} is not a TVM instruction. It is the architectural name for
a service-layer operation identifier that gives agents a shared vocabulary for economically
accountable action; the normative field is \texttt{operation\_id}. Metering does not require
a non-zero fee. Allowances, subscriptions, sponsorship, prepaid capacity and refundable bonds
are valid policies. The invariant is that resource use and externally imposed cost remain
bounded, attributable and enforceable.

The commercial beachhead is deliberately narrower than the architecture: paid model and tool
invocations across independent agent builders and providers, with Messenger and Mailbox as a
second profile that stress-tests delegation, receipts and anti-abuse economics. TOS reuses
HTTP, MCP, A2A, OpenAPI and compatible discovery systems rather than replacing them. Private
payloads and workloads remain off-chain; shared authority, commitments, disputes and final
settlement use TOS where counterparties need a tamper-resistant source of truth.
...
ADNL/DHT/RLDP networking. The generic Agentic Operation Protocol, SDKs, conformance suite and
most service profiles are partial or planned. Artificial Intelligence Proof of Work (AIPoW)
is feature-gated off by default: token incentives must not substitute for product-market fit,
organic paid demand or sustainable unit economics.
\end{abstract}
...
Web2 gave software APIs, but APIs alone are not sufficient for autonomous agents.

An API may describe how to format and transport a request, yet it usually does not provide a
portable answer to the questions an autonomous agent must resolve before acting:

\begin{itemize}
  \item who is making the request and under whose authority;
  \item which exact operation and revision is being requested;
...
TOS Network is being built to standardize this missing layer. Its central abstraction is the
\textbf{Agentic Service Operation}: a typed, authorized, metered, receipted and settleable
action that can be implemented by independent providers without requiring them to share one
application platform or one operator.
...
\begin{enumerate}
  \item \textbf{TOS Core.} Actor execution, consensus, sharding, persistent identity,
        value-bearing messages, payment, naming and authenticated networking.
  \item \textbf{Agentic Operation Protocol.} Versioned operation identities, schemas,
        capabilities, replay protection, resource envelopes, quotes, budgets, metering,
        receipts, evidence, errors and settlement.
  \item \textbf{Discovery and naming.} ARD-compatible publication and search, TOS DNS,
        signed descriptors and independently operated registries.
  \item \textbf{Provider runtimes and relays.} Agent runtimes, Messenger relays, Mailboxes,
        model and tool servers, storage providers, search providers and owner-operated
        terminals.
  \item \textbf{Operation profiles.} Communication, compute, storage, events, commerce,
        booking, human services and physical-edge intelligence.
\end{enumerate}
...
Those systems can carry, describe or discover an interface. TOS adds the portable authority
and economic contract around the action: who may call, within what limits, at what maximum
cost, with what receipt and through what settlement path.

\subsection{Strategic Positioning}

The most compact positioning is:

\begin{quote}
\textbf{TOS Network turns ordinary Internet actions into typed, authorized, metered,
receipted and settleable operations for autonomous agents.}
\end{quote}
...
The unit of value is therefore a completed, policy-compliant Agentic Service Operation, not an
hour of unidentified hardware, an unbounded API call or an unauthenticated message. A consumer
requests an exact operation revision, input and output contract, deadline, region, data-egress
policy, resource limit, maximum charge and evidence level. A provider or target agent decides
whether it can safely admit the request, executes it under local policy, returns a signed
receipt and settles according to the accepted policy.
...
commercial beachhead is \textbf{paid model and tool invocation}: an agent discovers an
independent provider, receives a live quote, delegates a bounded budget, invokes a typed
operation, verifies a signed usage receipt and settles without creating a bilateral account
integration. This wedge has an immediate buyer, a measurable unit of work and a clear
...
If the beachhead works across independent implementations, the intended flywheel is:
...
\section{From APIs to Agentic Operations}
...
\subsection{Canonical Operation Lifecycle}
...
Every step is asynchronous. A quote may expire. Authorization may fail after discovery.
...
consensus outside hard real-time and actuator-control loops.
...
risk, permits operation profiles to evolve at application speed and keeps private workloads
outside validator execution.
...
\endhead
TOS Core &
Consensus, TVM, actor accounts, value-bearing messages, generic contracts, wallet,
cryptography, DNS and authenticated networking &
Implemented and maintained in the \texttt{tos} repository \\
Agentic Operation Protocol &
Operation identifiers, schemas, capabilities, replay protection, quotes, resource envelopes,
metering, receipts, evidence, error taxonomy and settlement &
Partial foundations and planned common specification, SDKs and conformance suite \\
Discovery and naming &
ARD publication and search, TOS DNS, signed descriptors, endpoint binding, indexes and
federated registries &
Core naming primitives implemented; product and compatibility layers partial or planned \\
Provider runtimes and relays &
Agent runtimes, Messenger relays, Mailboxes, model and tool services, storage, search and
owner-operated terminals &
Independently released application repositories and operator infrastructure \\
Operation profiles &
Communication, compute, storage, events, commerce, booking, human services and physical edge &
Independently versioned profile specifications and implementations \\
\bottomrule
\end{longtable}
...
continuous device telemetry do not belong in the validator repository.
...
The Agentic Operation Protocol is the common application plane above TOS Core. Its purpose is
not to define every business workflow. It defines the portable contract around an operation
so independent agents and providers can interoperate safely.
...
Freeze the common architecture with requirements from a first-party reference application,
...
\subsection{Phase 1: Compute Beachhead and Communication Validation}

Deliver the paid model and tool profile as the initial commercial wedge:
...
TOS Network is being built as \textbf{The Open System for the Agentic Internet}.

Its purpose is not to execute every Internet service inside blockchain consensus. Its purpose
is to give independently operated agents a common operation vocabulary and a common economic
execution contract.

Under this model, a meaningful Internet action becomes a versioned operation with explicit
identity, bounded authority, finite resource limits, a maximum budget, defined failure
behavior, a verifiable receipt and a settlement path. Communication, model inference, tool
invocation, storage, search, event delivery, commerce, booking, human services and
physical-edge intelligence are different profiles of the same operation model.

Private payloads and workloads remain where they belong: in encrypted sessions, provider
runtimes, local storage and owner-controlled terminals. TOS Core supplies shared actor state,
value, commitments, disputes and final settlement where independent parties need a
tamper-resistant source of truth.

The architecture separates three layers that are often confused. TVM opcodes execute inside
the deterministic blockchain VM. Contract message opcodes identify actor-level state
transitions. Agentic Service Opcodes identify economically accountable external actions.
Their shared name reflects a common insight---agents need a portable instruction vocabulary---
not a requirement that every action run in one virtual machine.

Metering is equally important. It does not mean every trusted message or local action must
incur a non-zero charge. It means no caller can impose unlimited storage, compute, delivery,
attention or retry costs without identity, capability, quota, budget or bond. This is the
foundation for open interoperability without making the Agentic Internet an open spam relay.
...
feature-gated AIPoW path establish important pieces. The open operation namespace, common
envelopes, provider SDKs, communication and compute conformance, high-frequency settlement and
additional profiles remain work to specify, implement, test and independently review.

The investable thesis is simple but demanding. If agents become independent economic actors,
the scarce layer shifts from generating intelligence to granting authority, bounding cost,
verifying outcomes and resolving failure across organizations. A protocol that becomes the
default contract for those actions can sit beneath many applications, providers and payment
methods. TOS has a differentiated architecture for that outcome, but architecture alone is
not a moat. The proof must come from a narrow compute beachhead, repeat organic demand,
third-party embedding, cross-provider completion and a measured advantage for TOS settlement.

Token issuance follows that proof; it does not create it. AIPoW remains inactive until product,
economic, security and decentralization gates are independently visible. This sequencing
preserves the larger vision while keeping the next milestone concrete: enable one agent to
buy one useful operation from an unrelated provider under a portable authority and receipt
contract, then make that flow repeat across providers without special integration.

Web2 standardized APIs for software. TOS aims to standardize economically accountable actions
for autonomous agents.

\begin{center}
\textbf{Web2 gave machines APIs. TOS gives agents economic opcodes.}
\end{center}
