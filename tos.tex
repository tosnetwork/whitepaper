...
conformance suites and most service profiles are partial or planned. TOS has a fixed maximum
supply of 500 million TOS. The bounded genesis bootstrap is included in that total; all
remaining native issuance comes only from finalized blocks produced by elected validators
through the protocol reward path. Service activity and off-chain scoring cannot create TOS.
\end{abstract}

\section*{Document Status}

This paper describes both released foundations and architectural direction. ``Implemented''
means released code contains the relevant node, contract, tooling or test
...
The architecture has four interoperable infrastructure layers surrounded by the participants
and economies that use them:

\begin{enumerate}
  \item \textbf{Agent Access, Coordination and Discovery.} Identity-aware entry points,
        authentication, authorization, capability publication and search, matching, quoting,
        spending policy, Agreement formation, job coordination, events and result delivery.
  \item \textbf{Autonomous Runtime and Provider Execution.} Agent runtimes, provider and
        worker runtimes, protocol and tool adapters, isolated execution, resource accounting,
        evidence capture and result generation.
  \item \textbf{Trust, Evidence and Economy.} Identity and capability commitments,
        reputation references, escrow, payment, Receipt verification, proof, dispute and
        optional demand-aligned contribution incentives.
  \item \textbf{Decentralized Network Foundation.} Validators, peer-to-peer propagation,
        consensus, the immutable ledger, smart contracts, event logs, state synchronization
        and finality.
\end{enumerate}

Users, personal and enterprise agents, autonomous applications and machines enter through the
access layer. Model, API, compute, storage, human and physical-service providers connect to
the runtime layer. Node operators, stakers and governance participants maintain the network
foundation. Applications and institutions may compose all four layers without becoming a
new source of protocol authority.
...
\begin{itemize}
  ...
  \item concentration of settled value, validator stake and validator rewards after
        common-control aggregation; and
  \item cumulative validator-led native creation against the fixed 500-million-TOS cap.
\end{itemize}

Subsidized circular volume, address count, message count and declared capacity are not
substitutes for these measures. Validator rewards are a consensus-security expenditure, not
evidence of organic demand for Agentic Service Operations.
...
\subsection{Opcode Registry and Maturity}
\label{sec:opcode-registry}

This edition names \textbf{25 distinct Agentic Service Opcode families}. These identifiers are
an architectural vocabulary and a development registry; naming an opcode in this paper does
not mean that its revision has been frozen as an interoperable TOS standard. Earlier drafts
also used \opid{model.infer@2} as an illustrative revision. This edition normalizes the
illustrative model-inference entry to \opid{model.infer@1}; a future incompatible semantic
change would advance the immutable revision in the normal way.

Opcode maturity is tracked independently from the implementation status of the underlying TOS
Core contracts and networking primitives:
...
\endhead
Design &
The operation name and intended semantics are described, but the canonical wire contract and
conformance vectors are not complete. \\
Partial &
One or more profile-specific implementations or reusable primitives exist, but the complete
canonical operation contract is not frozen. \\
Frozen &
Canonical identifier, revision, schemas, authority rules, metering, errors, Receipt semantics
and success/failure vectors are published and immutable for that revision. \\
Implemented &
A reference implementation passes the frozen conformance suite for that exact revision. \\
Interoperable &
At least two independently authored implementations exchange and verify equivalent requests,
results and Receipts for the frozen revision. \\
Production &
The interoperable revision has additionally passed the applicable security, release,
operational and public-network gates. \\
\bottomrule
\end{longtable}

As of this edition, \textbf{zero Agentic Service Opcode revisions have reached the Frozen,
Interoperable or Production maturity levels defined above}. Messenger and Mailbox have the
strongest profile-specific implementation foundations, but their generic operation contracts
remain Partial. The remaining opcode families are Design/Planned unless a released profile
specification says otherwise. This prevents an implemented low-level behavior from being
mistaken for a frozen Internet-wide semantic contract.

\begin{longtable}{L{0.30\textwidth}L{0.16\textwidth}L{0.43\textwidth}}
\toprule
Opcode & Maturity & Current scope \\
\midrule
\endhead
\opid{messenger.send@1} & Partial & Messaging-plane foundations exist; generic commercial and cross-provider contract not frozen. \\
\opid{messenger.introduce@1} & Design & Introduction/attention-bond semantics remain profile specification work. \\
\opid{messenger.acknowledge@1} & Partial & Acknowledgement foundations exist; portable Receipt semantics are not frozen. \\
\opid{mailbox.deposit@1} & Partial & Capability-bound deposit and stored-acknowledgement foundations exist. \\
\opid{mailbox.read@1} & Partial & Capability-bound read foundations exist; generic provider conformance is incomplete. \\
\opid{mailbox.delete@1} & Partial & Capability-bound delete foundations exist; generic provider conformance is incomplete. \\
\opid{mail.send@1} & Design & Mail-specific domain and anti-abuse profile remains planned. \\
\opid{model.infer@1} & Design & Model-provider quote, metering, evidence and Receipt profile remains planned. \\
\opid{embedding.create@1} & Design & Model/embedding profile remains planned. \\
\opid{speech.transcribe@1} & Design & Speech profile remains planned. \\
\opid{tool.invoke@1} & Design & Generic tool invocation profile remains planned. \\
\opid{workflow.execute@1} & Design & Workflow composition profile remains planned. \\
\opid{storage.put@1} & Design & Storage lease and durability profile remains planned. \\
\opid{storage.get@1} & Design & Storage retrieval profile remains planned. \\
\opid{storage.renew@1} & Design & Storage renewal profile remains planned. \\
\opid{storage.delete@1} & Design & Storage deletion/expiry semantics remain planned. \\
\opid{event.publish@1} & Design & Event fan-out, backlog and settlement profile remains planned. \\
\opid{event.subscribe@1} & Design & Subscription, replay and backlog profile remains planned. \\
\opid{commerce.quote@1} & Design & Commerce domain state machine remains planned. \\
\opid{commerce.order@1} & Design & Commerce order/fulfillment profile remains planned. \\
\opid{booking.reserve@1} & Design & Reservation, deposit and cancellation profile remains planned. \\
\opid{booking.cancel@1} & Design & Booking cancellation/refund semantics remain planned. \\
\opid{delivery.request@1} & Design & Delivery and fulfillment profile remains planned. \\
\opid{human.task@1} & Design & Human-service evidence and dispute profile remains planned. \\
\opid{task.post@1} & Design & Task-market operation wrapper remains planned above the implemented Task Escrow actor. \\
\bottomrule
\end{longtable}

The registry is intentionally extensible. Third parties may define new operation namespaces,
but a TOS publication must state its maturity explicitly. A Design or Partial identifier must
not be advertised as a frozen TOS standard, and a Frozen identifier must not be advertised as
Interoperable until an independent implementation demonstrates the same semantics.
...
\texttt{messenger.send@1}, \texttt{model.infer@1} &
...
operation model without sharing one workload implementation. The authoritative maturity
summary for the identifiers named in this edition is Section~\ref{sec:opcode-registry}.
...
\subsection{Layered TOS Network Architecture}

TOS Network is one system composed of four interoperable infrastructure layers. The layers
separate coordination, execution, economic trust and decentralized state so that each can
evolve at the appropriate speed. They are responsibility and authority boundaries, not
repository boundaries and not a requirement that one organization operate the whole stack.

\clearpage
\begin{landscape}
\begin{figure}[p]
\centering
\includegraphics[width=0.98\linewidth]{tos-network-architecture.png}
\caption{TOS Network as a layered infrastructure system. Private payloads and execution remain
off-chain; bounded authority, commitments, evidence and economic outcomes cross the shared
trust boundary only where required.}
\label{fig:tos-layered-architecture}
\end{figure}
\end{landscape}
\clearpage

\begin{longtable}{L{0.20\textwidth}L{0.39\textwidth}L{0.30\textwidth}}
\toprule
Layer & Primary responsibilities & Authority boundary \\
\midrule
\endhead
Agent Access, Coordination and Discovery &
Accept goals from heterogeneous agent clients; authenticate principals; apply owner spending
and capability policy; publish and discover services; match candidates; negotiate quotes;
form Agreements; invoke work; coordinate asynchronous jobs and return results &
May interpret, rank and route, but cannot create owner authority, silently expand an Agreement
or make an index authoritative \\
Autonomous Runtime and Provider Execution &
Adapt models, tools, APIs, local programs, compute and physical devices; admit jobs; isolate
workers; enforce time, memory, compute, network and data limits; meter resources; generate
results, evidence and signed Receipts &
Providers control admission, resources, data and safety; a runtime cannot self-authorize a
caller or declare its own economic statement final \\
Trust, Evidence and Economy &
Anchor identity and capability commitments; manage reputation references, budgets, escrow,
payments, Receipt verification, proofs, disputes, refunds and final settlement; enforce
fixed-supply accounting and validator-reward policy &
Economic state follows accepted policy and verified evidence; a gateway, provider, indexer or
analytics service cannot mint, move or finalize value by assertion \\
Decentralized Network Foundation &
Provide authenticated propagation, validator consensus, immutable actor state, contracts,
events, Receipt commitments, state synchronization and finality &
Consensus verifies deterministic state transitions and commitments; validators do not execute
private model, tool, software or physical-service workloads \\
\bottomrule
\end{longtable}

Applications and institutions sit above and across these layers. Communication, software
work, model calls, compute, storage, commerce, booking, human services and physical-edge work
are service profiles of the same infrastructure, not separate definitions of TOS Network.

\subsection{Four Cross-Layer Flows}

The architecture is easier to reason about as four flows that meet at explicit boundaries:

\begin{longtable}{L{0.20\textwidth}L{0.69\textwidth}}
\toprule
Flow & Meaning \\
\midrule
\endhead
Request and discovery &
An agent expresses Intent, resolves identity and capabilities, discovers candidates and
receives quotes. Search results and recommendations remain non-authoritative. \\
Job and execution &
An accepted Agreement becomes one or more bounded invocations. The provider admits and
executes work under local policy, isolates resources and returns the result. \\
Settlement and value &
Budget, escrow, usage and payment move according to the Agreement. The path may use native
assets, stable value, prepaid credit, sponsored allowances, payment channels or batched
settlement, provided the maximum external cost remains enforceable. \\
Trust and state &
Signatures, commitments, Receipts, proofs, disputes, events and final settlement connect
off-chain action to shared state. Raw private payloads do not need to enter consensus. \\
\bottomrule
\end{longtable}

The request and job flows are normally high-frequency and off-chain. The value and trust
flows touch shared state only as required by the accepted risk, evidence and settlement
policy. This keeps agent interaction fast without turning a gateway, runtime or index into a
trusted intermediary.

\subsection{End-to-End Intent-to-Settlement Path}

\begin{enumerate}
  \item A user, organization or autonomous agent expresses an Intent through any compatible
        client or runtime.
  \item The access layer authenticates the principal, resolves delegated authority and
        discovers providers that claim the required capability.
  \item Matching compares verifiable capability, price, latency, reputation, locality,
        evidence and policy compatibility; candidates return bounded quotes.
  \item Deterministic owner policy or explicit owner approval accepts precise terms and forms
        an Agreement. Discovery or model output alone cannot authorize spending.
  \item The coordination layer invokes the agreed operation and the provider runtime admits
        it under current capacity, legal, security and safety policy.
  \item An isolated worker executes within the agreed resource, time, tool, network and data
        bounds while producing the required evidence and metering record.
  \item The provider returns the result and a signed Receipt bound to the Agreement,
        operation, usage, evidence policy and result commitment.
  \item The verification and settlement path releases, nets, refunds or disputes value, and
        records only the shared commitments and outcomes required by the protocol.
  \item The verified result returns to the caller. Derived indexes and reputation views may
        update from the finalized outcome, while critical clients continue to verify source
        signatures and chain state.
\end{enumerate}

Existing Agent, Service, Escrow, Registry, Dispute and Attestation actors remain reusable
trust foundations. Product-specific daemons, message payloads, model runtimes, catalogs and
continuous device telemetry do not belong in validator execution. A Gateway, Carrier or
...
The Agentic Operation Protocol is the common execution contract spanning the Agent Access,
Coordination and Discovery layer, the Autonomous Runtime and Provider Execution layer and the
Trust, Evidence and Economy layer above the Decentralized Network Foundation. Its purpose is
not to define every business workflow. It defines the portable contract through which an
accepted Agreement authorizes one or more operations so independent agents and providers can
interoperate safely.
...
Four interoperable infrastructure layers spanning access and coordination, provider execution,
trust and economy, and the decentralized network foundation, with plural providers, indexes,
...
cannot create product-market fit. Validator rewards secure consensus; they are not service
subsidies and must not be reported as customer demand. TOS therefore evaluates operation
adoption, sustainable unit economics and independent provider participation separately from
validator issuance.
...
\begin{enumerate}
  ...
  \item win unrelated design partners and repeat payers without protocol-funded demand or
        service-linked token subsidies;
  ...
  \item operate validator rewards transparently within the fixed supply cap.
\end{enumerate}
...
The maximum gross native supply is fixed at exactly 500,000,000 TOS. The entire supply is
allocated through the validator-led network launch and reward path. A provisional 101,000 TOS
genesis bootstrap establishes the original validator set and the Elector and Configuration
contracts; it is included within the cap. The remaining 499,899,000 TOS may be created only
through finalized block production and distributed through the elected-validator reward path.
There is no separate service-work, proof-of-work, team, investor, foundation or treasury
issuance allocation. The production policy is:
...
Fixed maximum gross native supply & 500,000,000 TOS \\
Total validator-led allocation, including genesis bootstrap & 500,000,000 TOS \\
...
Reward termination & Creation stops at the fixed cap; ConfigParam 14 is tapered or set to zero \\
Required hard consensus supply cap & 500,000,000 TOS \\
\bottomrule
\end{longtable}

The 500-million-TOS limit is a monetary invariant, not a calibration target. Gross native
creation includes the genesis bootstrap and every later validator block reward, including
amounts subsequently burned. Consensus must reject any transition that would raise gross
native creation above the cap. The distribution duration remains an approximate policy target:
its completion date depends on finalized block production, shard behavior, validator
availability and configured reward values, but those variables cannot increase the maximum
supply.

\subsection{Genesis Bootstrap and Allocation}

Within the fixed 500-million-TOS cap, the production zerostate provisionally creates 101,000
TOS: a bounded 100,000-TOS
...
Post-genesis native creation uses the block-value and Elector path and is bounded by the
remaining headroom below the fixed 500-million-TOS cap:
...
creation increases gross native supply, while fees transfer existing TOS. No validator reward
may be created once the remaining cap headroom reaches zero.

\subsection{Validator Distribution Schedule}
...
An outage, partition, failed election or delayed restart creates no issuance debt. There is
...
Governance must monitor finalized creation and begin a public taper review before gross native
creation reaches 495 million TOS. It may reduce ConfigParam 14 to manage the approach to the
cap and must set both masterchain and basechain creation values to zero before the cap is
exhausted. Consensus-enforced remaining-supply arithmetic is the final backstop: the reward for
a block cannot exceed the remaining headroom. Transaction and service fees continue after
native creation stops. No governance or configuration action may restart creation above the
fixed 500-million-TOS maximum.
...
Ordinary authenticated configuration governance retains the ability to change block-reward
rates, validator counts, stake limits, election timing and other configurable protocol
parameters within the fixed supply invariant. Every monetary proposal should disclose its
proposer, old and proposed values, activation conditions, expected supply impact and recorded
votes. Governance can alter the rate and timing of validator distribution, but it cannot
authorize gross native creation above 500,000,000 TOS. Configuration authority remains a
material security and distribution risk even though the maximum supply is fixed.
...
For Agentic Service Opcodes specifically, this edition names 25 distinct operation families.
The maturity model in Section~\ref{sec:opcode-registry} is stricter than the implementation
status of reusable contracts: \textbf{no named Agentic Service Opcode revision is currently
Frozen, Interoperable or Production}. Messenger and Mailbox contain Partial profile-specific
foundations; the rest of the named operation families remain Design/Planned unless a released
...
Validator-led native creation and fixed supply &
\partialstatus &
ConfigParam 14 and Elector reward foundations exist; consensus-enforced inclusion of genesis
creation, remaining-cap clipping and the 500-million-TOS terminal boundary require release
...
\begin{itemize}
  ...
  \item actuator isolation, local safety override, fleet revocation and bounded command
        fan-out;
  \item validator-reward arithmetic, genesis inclusion, cumulative native-creation accounting,
        remaining-cap clipping and exact termination at 500,000,000 TOS; and
  \item adversarial reward-parameter changes, shard split and merge behavior, block-rate
        variation, outages, restarts and attempts to create supply after the cap is reached.
\end{itemize}
...
\subsection{Fixed-Supply Gate}

Before production launch, independently verify that zerostate creation and every validator
reward share one cumulative gross-supply counter and that no block, shard transition,
configuration change, restart or governance action can raise it above 500,000,000 TOS. Test
the final partial reward below the cap, the first block after exhaustion, reward-rate changes,
outages and shard split-and-merge behavior. A failed or delayed gate creates no issuance debt.
...
\begin{itemize}
  ...
  \item public reporting of quote conversion, repeat payers, completion, unit economics,
        settlement cost and receipt verification without protocol-funded demand or
        service-linked token subsidies.
\end{itemize}
...
\subsection{Cross-Cutting Monetary Gate}

Keep the fixed supply invariant independent of service activity. Validator rewards compensate
consensus participation and cannot depend on paid communication, model output, self-generated
traffic, provider claims or any off-chain score. Production launch requires independently
reproduced supply accounting, cap-boundary tests, transparent reward parameters and a public
plan for validator security after native creation stops.
...
The current opcode registry names 25 distinct Agentic Service Opcode families, but none has
yet reached the Frozen, Interoperable or Production maturity levels defined by this paper.
Messenger and Mailbox provide Partial implementation foundations; the remaining families are
design targets. This distinction is intentional: a portable Internet primitive becomes a TOS
standard only after its exact revision, schemas, authority, metering, Receipt semantics and
conformance behavior are frozen and independently reproduced.
...
validator reward path establish important pieces. The open Intent and Agreement schemas,
...
TOS coordination and settlement. Validator issuance secures consensus; it does not create
customer demand. Its rate and distribution must remain transparent, and gross native creation
must never exceed the fixed 500-million-TOS maximum.
