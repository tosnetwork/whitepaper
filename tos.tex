...
\begin{abstract}
TOS Network is being built as \textbf{The Open System for the Agentic Internet}: a common
operation, authorization, metering, receipt and settlement layer through which independently
operated agents can safely invoke one another and external services.

TOS models meaningful Internet actions---such as sending a message, depositing encrypted data
into a mailbox, invoking a model or tool, storing an object, publishing an event, reserving a
resource, performing a task or settling a purchase---as versioned \textbf{Agentic Service
Operations}. Each operation has an explicit identity, input and output contract, capability
scope, replay domain, resource envelope, maximum budget, metering rules, receipt semantics and
settlement policy.

In this paper, an \textbf{Agentic Service Opcode} is not a TVM instruction. It is an
architectural name for a service-layer operation identifier that gives autonomous agents a
shared vocabulary for economically accountable action. The normative protocol field is
\texttt{operation\_id}. Metering does not require every operation to charge a non-zero fee:
trusted allowances, subscriptions, sponsorship, prepaid capacity and refundable bonds are
valid policies. The invariant is that resource consumption and externally imposed cost remain
...
ADNL/DHT/RLDP networking. It includes foundations for Agent Accounts, concurrent Service
Actors, Task Escrow, Dispute, Capability Registry and Proof Attestation. It also contains a
feature-gated Artificial Intelligence Proof of Work (AIPoW) path that is inactive by default
and creates no TOS unless governance activates it after the launch gates in this paper.

The broader Agentic Operation Protocol, open operation namespace, discovery products and
communication, compute, storage, commerce and physical-edge profiles are independently
evolving product layers. Application workloads and private payloads remain off-chain; shared
authority, commitments, receipts, disputes and settlement use TOS where independently
operated parties require a tamper-resistant source of truth.
\end{abstract}
...
\subsection{Economic Wedge and Network Flywheel}

The initial proof should not attempt to standardize every Internet behavior at once. Two
reference profiles demonstrate the horizontal nature of the architecture:

\begin{itemize}
  \item \textbf{Communication:} Messenger and Mailbox operations show identity, capability,
        replay resistance, recipient policy, storage, delivery receipts and anti-spam
        economics.
  \item \textbf{Compute:} Model and tool operations show live quotes, multidimensional
        resource metering, bounded output, evidence and provider settlement.
\end{itemize}

If those profiles interoperate across independent implementations, the intended flywheel is:

\begin{center}
\texttt{open operation definitions -> competing providers -> portable receipts}\\
$\longrightarrow$ \texttt{better trust, price and availability -> more independent demand}\\
$\longrightarrow$ \texttt{more providers and operation profiles}.
\end{center}

The defensible asset is not a closed catalog or token issuance by itself. It is the portable
graph of operation identities, exact revisions, capabilities, quotes, receipts, disputes,
settlement and operating reliability. Because discovery and operation definitions are open,
advantage must come from safer completion, better economics and stronger evidence rather than
from locking agents into one platform.

\subsection{Falsifiable Metrics}

Before describing this system as a network effect, public reporting should show at least:

\begin{itemize}
  \item organic settled value and its share of all settled value;
  \item unrelated unique and repeat payer control domains;
  ...
  \item provider retention and non-reward service revenue;
...
infrastructure this layer requires. This is a structural thesis, not a claim about currently
realized transaction volume, protocol revenue or token value.
...
Freeze the common architecture:
...
\subsection{Phase 1: Communication and Compute Reference Profiles}

Deliver two independent profile families:

\begin{itemize}
  \item Messenger and Mailbox operations with E2EE payload handling, capability-bound
        deposit/read/delete, receipt levels, quotas and introduction-bond experiments; and
  \item model and tool operations with live quotes, token/time/tool metering, bounded
        execution and signed usage receipts.
\end{itemize}

Each profile requires at least two independently authored implementations or provider
adapters.
...
automatically qualify for issuance.
